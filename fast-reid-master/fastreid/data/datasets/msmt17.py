...
class MSMT17(ImageDataset):
    def __init__(self, root='datasets', **kwargs):
        self.train_path = osp.join(root,'msmt17/bounding_box_train')
        self.test_path = osp.join(root,'msmt17/bounding_box_test')
        self.query_path = osp.join(root,'msmt17/query')

        self.tmp_train=[]
        self.tmp_test=[]
        self.train=[]
        files = glob.glob(osp.join(self.train_path, '*.jpg'))
        for fpath in files:
            fname = osp.basename(fpath)
            pid,cid,_=fname.split("_")
            pid = int(pid)
            cid = cid[1:]
            cid = int(cid)-1
            self.tmp_train.append((fpath,'msmt17_'+str(pid),'msmt17_'+str(cid)))
        files = glob.glob(osp.join(self.test_path, '*.jpg'))
        for fpath in files:
            fname = osp.basename(fpath)
            pid,cid,_=fname.split("_")
            pid = int(pid)
            cid = cid[1:]
            cid = int(cid)-1
            self.tmp_test.append((fpath,'msmt17_'+str(pid),'msmt17_'+str(cid)))
        files = glob.glob(osp.join(self.query_path, '*.jpg'))
        for fpath in files:
            fname = osp.basename(fpath)
            pid,cid,_=fname.split("_")
            pid = int(pid)
        ...
        for fname,pid,cid in self.tmp_train:
            self.train.append((fname,pid,cid))
            pid_set1.add(pid)

        train1_pid_num=len(pid_set1)
        for fname,pid,cid in self.tmp_test:
            pid = pid.split('_')[1]
            pid = int(pid)
            self.train.append((fname,'msmt17_'+str(pid+train1_pid_num),cid))
            pid_set2.add(pid)
        train12_pid_num = len(pid_set1) + len(pid_set2)
        print('msmt17 train',len(pid_set1),'msmt17 test',len(pid_set2))
        for _,pid,_ in self.train:
            pid_setall.add(pid)
        print('msmt17',len(pid_setall))

        ###################################################################
        query=[]
        gallery=[]
        super(MSMT17, self).__init__(self.train, query, gallery, **kwargs)
