...
__all__ = ['ToTensor', 'RandomErasing', 'RandomPatch', 'AugMix','RandomShuffleChannel']
...
class RandomShuffleChannel(object):

    def __init__(self, probability=0.5):
        self.probability = probability

        self.channel_num = 3
        self.channels = list(range(self.channel_num))
        self.shuffles = itertools.permutations(self.channels)
        self.shuffles = list(self.shuffles)
        self.shuffle_num = len(self.shuffles)
        self.shuffles = np.array(self.shuffles)
    ...
    def __call__(self, img):
        ...
        shuffle = np.random.randint(1, self.shuffle_num) - 1
        shuffle = self.shuffles[shuffle]
        img = img[:, :, shuffle]
        return img
